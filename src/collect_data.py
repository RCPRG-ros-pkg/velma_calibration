...
class DataCollector():
    ...
    def __init__(self):
        ...
        self._sub_left_image_color  = message_filters.Subscriber("/stereo/left/image_color", Image)
        self._sub_left_image_rect   = message_filters.Subscriber("/stereo/left/image_rect_color", Image)
...
#            print "--> kinect sample received (this only prints once!)"
    ...
    def run(self):
        ...
        [T_kl, R_kl] = self.listener.lookupTransform('/left_kinect_rgb_optical_frame', '/head_tilt_link', rospy.Time(0))
        [T_kr, R_kr] = self.listener.lookupTransform('/right_kinect_rgb_optical_frame', '/head_tilt_link', rospy.Time(0))
        # Start service
        ...
        sio.savemat(self.calib_file_path + 'calibration_data.mat', { "p_img_l":p_img_l, "p_world_l":p_world_l, "head_joints_l":head_joints_l, "num_images_left":float(len(self.p_img_l)), "p_img_r":p_img_r, "p_world_r":p_world_r, "head_joints_r":head_joints_r, "num_images_right":float(len(self.p_img_r)), "p_img_kl":p_img_kl, "p_world_kl":p_world_kl, "head_joints_kl":head_joints_kl, "num_images_kinect_left":float(len(self.p_img_kl)), "p_img_kr":p_img_kr, "p_world_kr":p_world_kr, "head_joints_kr":head_joints_kr, "num_images_kinect_right":float(len(self.p_img_kr)), "T_l":T_l, "R_l":R_l, "T_kl":T_kl, "R_kl":R_kl, "T_kr":T_kr, "R_kr":R_kr})
    ...
    def _capture_and_pub(self, sample_id, target_id, chain_id, pattern_size, square_size):
        ...
        while (not self._left_received or not self._kinect_left_rgb_received or not self._kinect_right_rgb_received or not self._head_joint_msg_received):
            rospy.sleep(0.005)
